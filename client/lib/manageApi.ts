...
//               so it is dropped on save — like `blob:` photos, an unstorable
//               field the profile does not round-trip.
...
// Save the four editor sets via their set-replace PUTs. Runs sequentially so a
// mid-save failure surfaces one clear error rather than racing four writes; each
// PUT is last-write-wins server-side (single owner). `blob:` photos are stripped
// by the editor before this is called (they cannot survive a reload).
export async function saveEditsToApi(edits: AthleteEdits): Promise<void> {
