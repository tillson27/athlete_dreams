...
// The directory contract omits `heroMediaUrl` on the profile, so callers thread
// the roster/directory hero through; falls back to the first gallery ref.
...
export function profileToMockAthlete(
  ...
  return {
    ...
    heroMediaUrl: heroMediaUrlFallback || (profile.gallery?.[0] ? unsplashPhoto(profile.gallery[0], 1400) : ''),
