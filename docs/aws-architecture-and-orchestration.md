...
- **Web build note (M3):** the client now has separate static-export modes: `GITHUB_PAGES=true` keeps the legacy `/athlete_dreams` basePath, while `STATIC_EXPORT=true` builds the S3/CloudFront domain export with **no basePath** for `athletearc.ca`. Once directory data is API-driven, static pre-rendering stops reflecting new athletes — the SSR/ISR move (Stage 2) becomes SEO-driven rather than optional.
