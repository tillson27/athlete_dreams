...
- `AthleteController`/`AthleteRouterFactory`: `PATCH /v1/athletes/me`, `POST /v1/athletes/me/{accomplishments,media,events}` (auth), `GET /v1/athletes/:slug/campaigns` (public). Change `GET /v1/athletes` to return `{items, nextCursor}`.
