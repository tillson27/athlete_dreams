...
| **M6** | Phase 4 — accounts, teams + client session cutover | ◐ Auth cutover + email verification/reset complete; Phase 4 hardening still pending | **Done:** client swapped `lib/session.ts`/follows/onboarding/dashboard/manage-editor mocks for real API auth in api mode (access-token-only sessions), added Resend-backed email verification/password reset, and filled the API gaps needed by onboarding/dashboard. **Pending:** refresh tokens/rotation, rate limiting, teams, and production sender/domain operations | Client on `test` runs sign-up → verify → onboard → publish → dashboard against the real API |
