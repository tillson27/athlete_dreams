...
| **M4** | Phase 2 — donations (Stripe **test mode**) | ◐ Implemented; hosted verification pending | Standard Connect onboarding, direct-charge donations, event ledger, Connect webhooks, payout observability (build sheet Phase 2). Remaining gate: run the flow against hosted Stripe test-mode credentials/webhooks. | Test-mode donation completes end-to-end on `test`; ledger + projections correct; webhook idempotency verified |
| **M5** | Phase 3 — campaign lifecycle | ◐ Status transitions implemented; transparency updates pending | Guarded campaign activation/completion/archival is implemented. Campaign update posting/listing and receipt-backed transparency workflows remain planned. | Lifecycle verified on `test` |
| **M6** | Phase 4 — accounts, teams + client session cutover | ◐ Auth cutover + email verification/reset complete; Phase 4 hardening still pending | **Done:** client swapped `lib/session.ts`/follows/onboarding/dashboard/manage-editor mocks for real API auth in api mode (access-token-only sessions), added Resend-backed email verification/password reset, added auth-route rate limiting, and filled the API gaps needed by onboarding/dashboard. **Pending:** refresh tokens/rotation, distributed rate limiting, teams, and production sender/domain operations | Client on `test` runs sign-up → verify → onboard → publish → dashboard against the real API |
