...
### Completion Notes
- **`client/lib/api.ts` (new):** typed fetch layer over `process.env.NEXT_PUBLIC_API_BASE_URL` — `fetchAthleteDirectory(params)`, `fetchAthleteProfile(slug)`, `fetchCommunityFeed(params)`, `fetchAthleteCampaigns(slug)`; each unwraps the `{ data }` / `{ error:{code,message} }` envelope and validates the payload against its `fad-common` schema, returning the parsed `fad-common` type or throwing a typed `ApiError` (`code` mirrors the API error envelope; sentinels `network_error`/`invalid_response`/`http_error`/`config_error`). No zod import in the client — the parser is accepted via a minimal structural `Parser<T>` type. All request/response types import from `fad-common` (no duplication).
- **Data-source seam (`client/lib/dataSource.ts`, `'use client'`):** exports `DATA_SOURCE` (`mock` default, `api` when `NEXT_PUBLIC_DATA_SOURCE=api`) and three hooks that expose the SAME existing view-model shapes the pages already consume — `useDirectoryAthletes()` → `MockAthlete[]`, `useCommunityData()` → `{ FeedItem[], RacingSoon[] }`, `useAthleteProfileData(slug, initial)` → `{ athlete: MockAthlete, profile: RichAthleteProfile }`. Mock mode returns the mock modules **synchronously** (no fetch, no loading), so render output is unchanged; api mode fetches client-side via a small `useApiResource` helper. The pure loaders live in `client/lib/apiLoaders.ts` (framework-free, directly unit-verifiable) with the shared `ProfileView` type in `client/lib/dataSourceTypes.ts` (avoids a hook↔loader cycle).
- **Adapters (`client/lib/adapters.ts`, pure functions):** map `fad-common` DTOs → view shapes. `directoryItemToMockAthlete`; `profileToRichProfile` + `profileToMockAthlete` (reconstruct the full `RichAthleteProfile` by merging the normalized arrays with the profile's `presentation` blob — arc chapters, training, tones→career/more split, headings, followers, powerProfile, recentBackers — every read defensive since `presentation` is `z.record(z.unknown())`); `feedItemToView` + `feedItemToRacingSoon`. Directory and community are filtered to the runner sports (`RUNNING`/`TRACK_AND_FIELD`) so api mode matches mock mode's runners-only surfaces; the community loader builds a per-slug hero/avatar map from the (runners-only) directory (the feed contract carries no hero) and keeps only feed items whose athlete is in that map (guarantees every card has an avatar).
- **Page wiring (minimal):** `AthleteDirectory.tsx` and `CommunityClient.tsx` (already client components) swap their module-scope mock imports for the hooks; the profile page stays a server component (static params + metadata still from mock) and renders a new thin `'use client'` `AthleteProfileHydrator` that calls `useAthleteProfileData` to swap in API data client-side. `client/.env.example` documents `NEXT_PUBLIC_DATA_SOURCE` (+ existing `NEXT_PUBLIC_API_BASE_URL`). Static-params generation is ALWAYS mock (documented SSR gap, Context §2); the profile hero is preserved from the server-rendered mock athlete because the profile contract omits `heroMediaUrl`.
- **DECISION A (dev-runner fix, `app/package.json` only — no `app/src/**` touched):** reproduced the logged bug — `tsx watch src/index.ts` boots via esbuild, which strips decorator metadata, so tsyringe throws `TypeInfo not known for "HealthRouterFactory"` (the production `tsc`→`node dist/index.js` path works). Fixed `dev` to `npm run build && concurrently -k -n tsc,api ... "tsc -p tsconfig.json --watch ..." "node --watch dist/index.js"` — a one-time build (so `dist/index.js` exists) then a tsc watcher + `node --watch` compiled-output runner. Added `concurrently ^8.2.2` to app devDeps (dependency-reuse: the root already depends on the same major; a bare-`&` compose can't propagate SIGINT cleanly). Verified the fixed `npm run dev` boots and serves `/v1/health/ready` → `{"data":{"status":"ready"}}` (HTTP 200) with **zero** `TypeInfo not known` errors. **Step-6 known-issue trail closed:** the tsx-can't-boot-DI issue flagged in step 6 is resolved here; `npm run dev --prefix app` is now functional.
- **DECISION B (media ref-vs-URL, Zod-first in `common/`):** proved the break — the profile response's `gallery`/`media[].mediaUrl` carry **bare Unsplash refs** (e.g. `1594882645126-14020914d58d`) which failed the schemas' `.url()` (`athleteProfileSchema.safeParse` failed at `gallery[0]` "Invalid url"), and the feed nulled its `photoUrl`. Added `mediaRefSchema` to `common/src/zod/shared.ts` (string that is EITHER an absolute http(s) URL OR a bare media ref; documented as a public API contract) and applied it to the media-display fields: `athleteMediaSchema.mediaUrl`, `athleteProfileSchema.gallery`, `athleteDirectoryItemSchema.heroMediaUrl`, `campaignSummarySchema.heroMediaUrl`, `communityFeedItemSchema.photoUrl`, plus the symmetric write-path `updateAthleteProfileRequestSchema.heroMediaUrl` + `setAthleteGalleryRequestSchema.gallery`. Genuine external URLs (`resultUrl`, `links[].href`, `socialStravaUrl`) stay `.url()`. Rebuilt `common`; the client composes display URLs via `unsplashPhoto` whenever a ref isn't already absolute (directory/community avatars + hero, profile gallery via the existing components). After relaxation the live profile/directory/feed responses all schema-parse (verified). The app's `RUN_DB_TESTS=1` suite still **fully passes (58/58)** — the relaxation broke no API contract test. **Note:** the feed's `photoUrl` remains null for now because Step 11's `CommunityFeedService.toPhotoUrl` guard (in `app/src/`, out of this step's edit scope) still nulls bare refs; the relaxed contract now *permits* refs so a one-line app follow-up can emit them — documented gap, no client regression (mock/api feed rosters match; api-mode feed photos are absent until that follow-up).
- **Extra Zod-first fix found during live verification:** `GET /v1/athletes/:athleteSlug/campaigns` returns a **bare array** of `CampaignSummary` (not a paginated wrapper). Added the named contract `athleteCampaignsResponseSchema = z.array(campaignSummarySchema)` to `common/src/zod/campaign.ts` and used it in `fetchAthleteCampaigns`. No `app/src` change (the app already returns that shape).
- **Live verification (honest):** built the API (`npm run build --prefix app`), ran `node app/dist/index.js` against the seeded Postgres (7 published athletes), and exercised every `api.ts` helper AND the three page loaders via tsx scripts (deleted after) with `NEXT_PUBLIC_DATA_SOURCE=api` + `NEXT_PUBLIC_API_BASE_URL=http://localhost:4000`. Results: **directory** — 7 items, runners-only filter → 6 (matches the mock roster: maya/felix/priya/jordan/cassandra/emma), each adapted card has slug+name+http hero; filtered `sport=TRACK_AND_FIELD` → priya-shah. **profile** — full high-fidelity reconstruction from live data (e.g. cassandra: handle `@cassandradewinter`, followers `12.4k`, disciplineLabel `Elite Endurance & Trail`, 4 PBs, 5 arc chapters, 2 careerHighlights, 3 previousRaces, 3 roadmap, 4 gallery, supportEnabled, 3 recentBackers); the naomi cyclist profile (bare-ref gallery) parses. **community** — 28 feed items → 24 runners-only, every card has an http avatar + headline, all 3 categories present, `racingSoon` correctly derived (e.g. "Jordan Blackhorse: Western States 100 (June 27, 2026)"); `category=race` filter → 14 (all race). **campaigns** — `fetchAthleteCampaigns(maya-okafor)` → 1 campaign ("Get me to Tokyo Marathon 2026", target 4200000). **error path** — unknown slug → `ApiError code=not_found status=404`. The client dev server in api mode serves `/athletes`, `/community`, `/athletes/maya-okafor` all HTTP 200 and compiles the client bundles. **Not directly verifiable here:** the post-hydration in-browser DOM swap (no headless browser available in this env) — the api-mode pages server-render the synchronous mock fallback, then the client bundle runs the exact fetch+adapt+load chain proven above; that chain is fully verified at the data layer.
- **Mock-mode byte-identity (proven):** stripping all Next infrastructure (script tags, `_next` asset paths, buildIds, hash tokens) from a `GITHUB_PAGES=true` export, **0 of 34** pages' rendered DOM differ between the branch base and this change. The raw HTML byte differences are solely Next's per-build nondeterministic artifacts (random `BUILD_ID` + content-hashed chunk filenames) — proven by building identical code twice (also 34/34 raw-differ). `npm run build` (default) still prerenders every route static and SSGs the profile pages from mock; GitHub Pages export unaffected.
- `$frontend-review` (`/frontend-review`, uncommitted + client focus) — aligned with `client/AGENTS.md`: types only from `fad-common`; no money display added (adapters carry `*AmountCents` but formatting stays in components via `formatCents`); server-components-by-default preserved (only the required hydrator is `'use client'`); story-first/minimalism UI untouched; removed dead code (`asStringArray`, unused `campaignSummaryHeroUrl`); comments are why/intent + public-API-contract only. `common/` changes checked against `common/AGENTS.md` (schemas defined here first, built, `.strict()` request bodies unchanged, cents unchanged).
- `$ci` (`/ci`) green (exit 0): common build, type-check across common/app/client, lint:fix "No ESLint warnings or errors", full build (incl. client Next build, all routes static), app tests. `RUN_DB_TESTS=1 npx vitest run` from `app/` → **6 files, 58 tests passed**. Lockfile drift is exactly `concurrently` + its transitive deps (intended); no AGENTS/mirror/build-artifact drift.
