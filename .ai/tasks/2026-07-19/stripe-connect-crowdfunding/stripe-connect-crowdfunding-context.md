...
- **Amount entry decision (settled):** collect the amount on the ARC donate widget and pass a fixed `unit_amount` to Checkout — higher conversion and the Stripe page arrives pre-filled. Concrete UX: three **hardcoded client-side** preset chips **$25 / $50 / $100** plus a **custom** amount field, all in **CAD**. Minimum donation is **$5** (`DONATION_MINIMUM_CENTS = 500`), enforced server-side on `POST /v1/donations` regardless of client input (Stripe also enforces its own ~$0.50 CAD floor). Presets are not per-campaign configurable for now (no amount-preset column on `Campaign`). Variant (documented, flippable — **not** a one-line change): to let the donor type the amount on Stripe's page, `custom_unit_amount` is a property of the **Price object**, not of inline `line_items[].price_data` (verified against live Stripe docs 2026-07-20). Flipping would require creating a Price with `custom_unit_amount: { enabled: true, minimum, maximum, preset }` and referencing it by id in the session — a small but real addition, not a field swap on `price_data`. The chosen fixed-`unit_amount` path is unaffected.
...
- **Direct-charge Checkout** — `stripe.checkout.sessions.create(params, { stripeAccount })` sets the `Stripe-Account` header; `mode: 'payment'`, `submit_type: 'donate'`; chosen amount via inline `line_items[].price_data.unit_amount` (fixed cents; `unit_amount` and `unit_amount_decimal` are the mutually-exclusive inline options). **Correction (verified live 2026-07-20):** donor-typed amounts are **not** an inline `price_data.custom_unit_amount` field — `custom_unit_amount` belongs to the **Price object**, so that variant needs a pre-created Price referenced by id (see §8). **Zero platform fee = omit `application_fee_amount`**; the connected account pays Stripe's processing fee. Source: `/connect/direct-charges`, `/api/checkout/sessions/create`, `/payments/checkout/pay-what-you-want`, `/api/prices/object`.
...
- **Flags to confirm in test mode:** exact `apiVersion` string baked into the installed `stripe-node` build (pin ours explicitly regardless — the SDK default has moved to `2026-07-08.preview`, so an explicit GA pin is required, not optional). `custom_unit_amount` placement resolved: it is a **Price-object** field, not inline `price_data` (only relevant if the donor-typed-amount variant is ever chosen).
